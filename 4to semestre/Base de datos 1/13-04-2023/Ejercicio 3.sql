...
select p.nombre profesor, grado
from (
select pgru.clave_prof clave_prof, g.nombre grado
from profesor_grupo pgru
join salones s on (pgru.id_curso = s.id_curso)
join grados g on (g.id_grado = s.id_grado)
group by pgru.clave_prof, g.nombre
)
group by p.nombre, grado
order by 1
